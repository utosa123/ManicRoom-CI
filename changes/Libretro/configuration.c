...
/* Own the bytes: callers commonly pass temporary NSString.UTF8String storage. */
static char g_custom_save_dir[PATH_MAX_LENGTH];
void set_custom_save_dir(const char *dir) {
   if (!dir || strlcpy(g_custom_save_dir, dir, sizeof(g_custom_save_dir)) >= sizeof(g_custom_save_dir))
      g_custom_save_dir[0] = '\0';
}

const char* get_custom_save_dir(void) {
   return g_custom_save_dir[0] ? g_custom_save_dir : NULL;
}
