...
void SetUserPath(const std::string& path) {
    ...
    // Existing emplace() entries otherwise retain the old NAND/SDMC path after
    // a prior GetUserPath() initialized defaults or a different game unloaded.
    if (!path.empty()) g_paths.clear();
#endif
